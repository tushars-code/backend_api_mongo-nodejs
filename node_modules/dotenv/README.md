...
```
```sh
$ dotenvx run --debug -- node job_entries.js
...
$ echo "console.log('Hello ' + process.env.HELLO)" > job_entries.js

$ dotenvx run --env-file=.env.production -- node job_entries.js
Hello production
> ^^
```
...
$ dotenvx run --env-file=.env.local --env-file=.env -- node job_entries.js
Hello local
```
