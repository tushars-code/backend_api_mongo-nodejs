...
```
```sh
$ dotenvx run --debug -- node user_profiles.js
...
$ echo "console.log('Hello ' + process.env.HELLO)" > user_profiles.js

$ dotenvx run --env-file=.env.production -- node user_profiles.js
Hello production
> ^^
```
...
$ dotenvx run --env-file=.env.local --env-file=.env -- node user_profiles.js
Hello local
```
